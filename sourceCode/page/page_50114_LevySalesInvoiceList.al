page 50114 "Levy Sales Invoice List"
{
    ApplicationArea = Basic, Suite;
    Caption = 'Levy Invoices';
    CardPageID = "Levy Sales Invoice";
    DataCaptionFields = "Sell-to Customer No.";
    Editable = false;
    PageType = List;
    PromotedActionCategories = 'New,Process,Report,Release,Posting,Invoice,Request Approval,Navigate';
    QueryCategory = 'Levy Invoice List';
    RefreshOnActivate = true;
    SourceTable = "Sales Header";
    SourceTableView = WHERE("Document Type" = CONST(Invoice), "Levy Invoice" = filter(true));
    UsageCategory = Lists;
    ...
    actions
    {
        area(navigation)
        {
            group("&Invoice")
            {
                ...
                action(PaymentInvoice)
                {
                    ...
                    Promoted = true;
                    PromotedCategory = Category6;
                    ToolTip = 'Update payment for the invoice.';
                    RunObject = page "Levy Payment Invoice";
            ...
            action("Posted Sales Invoices")
            {
                ...
                PromotedOnly = true;
                PromotedCategory = Category8;
                RunObject = Page "Posted Levy Invoices";
        ...
        area(processing)
        {
            ...
            group("P&osting")
            {
                ...
                action(AssessmentLetterPreview)
                {
                    ApplicationArea = Basic, Suite;
                    Caption = 'Assessment Letter Preview';
                    Image = Print;
                    Promoted = true;
                    PromotedCategory = Category5;
                    ShortCutKey = 'Ctrl+Alt+F9';
                    ...
                    begin
                        ...
                        Report.RunModal(Report::"Assessment Letter Levy", true, false, SalesHeader);
                    end;
                }
                action(SendEmail)
                {
                    ApplicationArea = Basic, Suite;
                    Caption = 'Release Assessment Letter';
                    Image = Attachments;
                    Promoted = true;
                    PromotedCategory = Category5;
                    ShortCutKey = 'Ctrl+Alt+F9';
                    ...
                    begin
                        EmailNotification.AssessmentLetter(Rec);
                    end;
                }
                action("Send Mail")
                {
                    ApplicationArea = All;
                    Caption = 'Send Mail for 97 Days';
                    Image = SendMail;
                    trigger OnAction()
                    var
                        MailNotify: Codeunit EmailNotification;
                    begin
                        MailNotify.LevysendOverdue97();
                    end;
                }
                action("Send Mail90")
                {
                    ApplicationArea = All;
                    Caption = 'Send Mail for 90 Days';
                    Image = SendMail;
                    trigger OnAction()
                    var
                        MailNotify: Codeunit EmailNotification;
                    begin
                        MailNotify.LevysendOverdue90();
                    end;
                }
            }
        }
