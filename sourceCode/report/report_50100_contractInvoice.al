report 50100 "Contract Invoice"
{
    DefaultLayout = RDLC;
    RDLCLayout = './sourceCode/reportLayout/Layout_50100_contractInvoice.rdl';
    PreviewMode = PrintLayout;
    Caption = 'Contract Invoice';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        var
                            ...
                            RecProjectDtl: Record "Project Details";
                        begin
                            ...
                            Clear(RecCustomer);
                            RecProjectDtl.Reset();
                            RecProjectDtl.SetRange("Customer No.", "Sales Invoice Line"."Sell-to Customer No.");
                            if RecProjectDtl.FindFirst() then;

                            RecRegDetail.Reset();
                            RecRegDetail.SetRange("Customer No.", "Bill-to Customer No.");
                            RecRegDetail.SetRange("Posted Invoice No.", "Sales Invoice Line"."Document No.");
                            if RecRegDetail.FindSet() then;
                            VarLinesValText[2] := Description + '- ' + RecRegDetail.Category + '- ' +
                            RecProjectDtl.Grade;
                            ...
                            VarLinesValDec[1] := Quantity;
                            VarLinesValDec[2] := "Unit Price";
                            VarLinesValDec[3] := "Line Amount";
                            VarLinesValDec[4] := 0;
                            //end;
                        end;
                    }
            ...
            begin
                ...
                VarHeadInfo[1] := Format("Posting Date", 0, '<Day,2>/<Month,2>/<Year4>');
                VarHeadInfo[2] := "No.";
                VarHeadInfo[3] := 'Registration & Subscription Fee';
                ...
                RecProject.SetRange("Invoice No.", "No.");
                if RecProject.FindFirst() then;
                if RecProject."Levy payment option" = 'PAID by Contractor' then begin
                    VarBillTo[1] := RecProject."POC Contact Name";
                    VarBillTo[2] := RecProject."Project Number";
                    VarBillTo[3] := RecProject.getPostalAddress();
                end else begin
                    VarBillTo[1] := RecCust.Name;
                    VarBillTo[2] := RecCust."No.";
                    VarBillTo[3] := RecCust.Address;
                    // VarBillTo[4] := RecCust."Mobile Phone No.";
                    //VarBillTo[5] := RecCust."E-Mail";
                end;
                // VarBillTo[5] := RecCust."Address 2";

                // if RecCust.City <> '' then begin
                //     RecCountry.Reset;
                //     if RecCountry.Get(RecCust."Country/Region Code") then
                //         VarBillTo[6] := "Bill-to City" + ', ' + RecCountry.Name + ' ' + RecCust."Post Code";
                // end else begin
                //     RecCountry.Reset;
                //     if RecCountry.Get(RecCust."Country/Region Code") then
                //         VarBillTo[6] := RecCountry.Name + ' ' + RecCust."Post Code";
                // end;

                // if RecContact.Get("Bill-to Contact No.") then begin
                //     VarBillTo[7] := 'Email ' + RecContact."E-Mail";
                //     VarBillTo[8] := 'Phone ' + RecContact."Phone No.";
                // end;
    ...
    var
        ...
        RecSlsSetup: Record "Sales & Receivables Setup";
        RecContact: Record Contact;
        ReqPDocTittleOpt: Option "Invoice";
